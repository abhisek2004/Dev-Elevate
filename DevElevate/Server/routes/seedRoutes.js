...
// Seed quizzes endpoint (admin only)
router.post("/seed-quizzes", authenticateToken, authorize(["admin"]), async (req, res) => {
  try {
    ...
    if (existingQuizzes > 0) {
      return res.status(400).json({ 
        message: `Database already contains ${existingQuizzes} quizzes. Use force=true to overwrite.`,
    ...
    res.status(201).json({
      message: `Successfully seeded ${insertedQuizzes.length} quizzes`,
      count: insertedQuizzes.length,
      quizzes: insertedQuizzes.map(quiz => ({
        id: quiz._id,
        title: quiz.title,
        topic: quiz.topic,
...
// Force seed quizzes (overwrites existing)
router.post("/seed-quizzes/force", authenticateToken, authorize(["admin"]), async (req, res) => {
  try {
    // Find an admin user to assign as creator
    let adminUser = await User.findOne({ role: "admin" });
    ...
    res.status(201).json({
      message: `Successfully replaced ${deletedCount.deletedCount} quizzes with ${insertedQuizzes.length} new quizzes`,
      deletedCount: deletedCount.deletedCount,
      insertedCount: insertedQuizzes.length,
      quizzes: insertedQuizzes.map(quiz => ({
        id: quiz._id,
        title: quiz.title,
        topic: quiz.topic,
    ...
    console.error("Error force seeding quizzes:", error);
    res.status(500).json({ 
      message: "Internal Server Error",
      error: error.message 
    });
...
router.get("/seed-status", async (req, res) => {
  try {
    ...
    res.status(200).json({
      quizCount,
      adminCount,
      canSeed: adminCount > 0,
      message: quizCount > 0 ? 
        `Database contains ${quizCount} quizzes` : 
