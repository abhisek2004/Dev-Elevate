      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {modules.map((module, index) => {
          ...
          return (
            <div
              key={module.id}
              className={`p-6 rounded-lg border group transition-all duration-200 ${state.darkMode ? 'border-gray-700 bg-gray-800 hover:shadow-lg' : 'border-gray-200 bg-white hover:shadow-md'
                } hover:shadow-md transition-shadow`}
            >
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-4">
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center font-semibold text-sm shadow-sm ${module.completed
              ...
              <div className="mb-4">
                ...
                <ul className="space-y-1.5">
                  {module.topics.map((topic, topicIndex) => (
                    <li
                      key={topicIndex}
                      className={`text-sm flex items-center gap-2 ${state.darkMode ? 'text-gray-400' : 'text-gray-600'
                        }`}
                    >
                      <span
                        className={`w-2 h-2 rounded-full ${state.darkMode ? 'bg-gray-500' : 'bg-gray-400'
                          }`}
                      ></span>
              ...
              <div className="flex flex-wrap gap-3">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors shadow-sm ${module.completed
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg border font-medium transition-colors shadow-sm ${state.darkMode
                      ? 'border-gray-600 text-gray-300 hover:bg-gray-700'
                      : 'border-gray-300 text-gray-700 hover:bg-gray-100'
                    }`}
                >
