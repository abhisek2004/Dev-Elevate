      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {isLoading ? (
            ...
            filteredJobs.map((job: JobOpportunity, index) => (
              <div
                key={index}
                ...
                <div className="flex justify-between items-center mt-4">
                  <div>
                    ...
                    <p className={`text-xs mt-1 ${state.darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      {job.deadline ? `Deadline: ${job.deadline}` : `Posted: ${new Date(job.postedDate).toLocaleDateString()}`}
                    </p>
                  </div>
                  <button
                    onClick={async () => {
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap items-center  gap-4 text-sm">
                      <span className={`${state.darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                        📄 {resource.type} • {(resource.fileSize / 1024 / 1024).toFixed(2)} MB
                      </span>
                      <span className={`${state.darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                      ...
                      onClick={async () => {
                        try {
                          ...
                          if (resource.fileUrl) {
                            ...
                            alert(`❌ Download link not available for ${resource.title}`);
                          }
                        } catch (error) {
